...
import { ongoingCamMov } from "../../components/SpaceRenderer/SpaceRenderer"

// TODO refactor: this should be a class extending a generic Stage class,
// With draw, mouseclicked, load and unload functions
export default function _drawSpaceNavigationStage(p5, skyboxImg, celestialObjects, cam) {
  ...
  // );
  celestialObjects.sort(
    (p1, p2) => p2.getDistWithCam(cam, p5) - p1.getDistWithCam(cam, p5)
  );
  celestialObjects
    .filter((o) => o.isInCluster("alpha"))
    .forEach((o) => o.draw(cam, p5));

  // Move camera
  handleCameraMovement(p5, cam);
}

let oldCamPos = [0, 0, 0];

function handleCameraMovement(p5, cam) {
  if (ongoingCamMov && !ongoingCamMov.isEnded(p5)) {
    ongoingCamMov.tick(p5);
  ...
  p5.perspective(p5.PI / 3, p5.width / p5.height, 1, SKYBOX_RADIUS * 2);
  ...
  if (p5.dist(cam.eyeX, cam.eyeY, cam.eyeZ, 0, 0, 0) > p5.SKYBOX_RADIUS * 0.75) {
