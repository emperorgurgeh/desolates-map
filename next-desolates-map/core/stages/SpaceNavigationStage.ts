...
export default function _drawSpaceNavigationStage() {
    const { skyboxImg, celestialObjects } = SpaceRenderer.getInstance();
    ...
    // MAXWELL: Sort doesnt exist on window.p5 or our instance
    celestialObjects.sort(
        (p1, p2) => p2.getDistWithCam() - p1.getDistWithCam()
    );
    celestialObjects
...
function handleCameraMovement() {
    const { ongoingCamMov } = SpaceRenderer.getInstance();
    const p5 = SpaceRenderer.getInstance().p5!;
